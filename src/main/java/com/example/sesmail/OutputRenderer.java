...
public class OutputRenderer {
    ...
    public OutputRenderer(String format) {
        this.format = (format != null && format.equalsIgnoreCase("json")) ? "json" : "text";
    }
    ...
    public void renderDryRun(AppConfig config, String mimeHeaderPreview) {
        if ("json".equals(format)) {
            ...
            System.out.println("  \"awsRegion\": " + jsonString(config.awsRegion()) + ",");
            System.out.println("  \"from\": " + jsonString(config.from()) + ",");
            System.out.println("  \"to\": " + jsonString(config.to()) + ",");
            System.out.println("  \"subject\": " + jsonString(config.subject()) + ",");
            ...
            if (config.sesConfigurationSet() != null && !config.sesConfigurationSet().isBlank()) {
                System.out.println("SES Config Set: " + config.sesConfigurationSet());
            }
            System.out.println();
            System.out.println("NOTE: SES prerequisites (verified identity, sandbox status, DNS) are not");
            System.out.println("      checked in dry-run mode. These must be configured externally.");
        }
    }
    ...
    public void renderSuccess(String messageId, AppConfig config) {
        ...
        if ("json".equals(format)) {
            ...
            System.out.println("  \"sesMessageId\": " + jsonString(messageId) + ",");
            System.out.println("  \"from\": " + jsonString(config.from()) + ",");
            System.out.println("  \"to\": " + jsonString(config.to()) + ",");
            System.out.println("  \"region\": " + jsonString(config.awsRegion()) + ",");
    ...
    public void renderVerboseConfig(AppConfig config) {
        ...
        System.out.println("[verbose]   from:       " + nullSafe(config.from()));
        System.out.println("[verbose]   to:         " + nullSafe(config.to()));
        System.out.println("[verbose]   region:     " + nullSafe(config.awsRegion()));
        System.out.println("[verbose]   dryRun:     " + config.dryRun());
        System.out.println("[verbose]   output:     " + nullSafe(config.outputFormat()));
        // Secrets are never printed - AWS credentials are handled by the SDK credential chain
        System.out.println("[verbose]   AWS credentials: resolved by AWS SDK default credential chain");
        System.out.println("[verbose]   AWS_ACCESS_KEY_ID: " + maskEnv("AWS_ACCESS_KEY_ID"));
        System.out.println("[verbose]   AWS_SESSION_TOKEN: " + maskEnv("AWS_SESSION_TOKEN"));
    }
