...
public class SesMailSender {
     ...
     * @param sesConfigurationSet   optional SES configuration set name; may be null
     */
    public SesMailSender(String region, String endpointOverride, String sesConfigurationSet) {
     ...
     */
    public String send(byte[] rawMimeBytes, String configurationSetName) throws SesMailException {
        try {
            ...
            if (configurationSetName != null && !configurationSetName.isBlank()) {
                requestBuilder = requestBuilder.configurationSetName(configurationSetName);
            }
            ...
            throw new SesMailException(
                    "ERROR: sender identity not verified in SES region. "
                            ...
                            + "Detail: " + e.getMessage(), e, ExitCode.SES_REJECTION);
        } catch (AccountSuspendedException | SendingPausedException e) {
            throw new SesMailException(
                    "ERROR: SES account sending is suspended or paused: " + e.getMessage(),
                    e, ExitCode.SES_REJECTION);
        } catch (software.amazon.awssdk.core.exception.SdkClientException e) {
            throw new SesMailException(
                    "ERROR: AWS client configuration failure (check region, credentials, network): "
                            + e.getMessage(), e, ExitCode.AUTH_ERROR);
            ...
            if (e.getMessage() != null && e.getMessage().contains("sandbox")) {
                throw new SesMailException(
                        "ERROR: account appears to be in SES sandbox; recipient must be verified. "
                                + e.getMessage(), e, ExitCode.SES_REJECTION);
            }
            if (e.awsErrorDetails() != null
                    && "InvalidClientTokenId".equals(e.awsErrorDetails().errorCode())) {
                throw new SesMailException(
                        "ERROR: invalid AWS credentials: " + e.getMessage(),
                        e, ExitCode.AUTH_ERROR);
            }
            throw new SesMailException("ERROR: SES API error: " + e.getMessage(), e, ExitCode.SES_REJECTION);
        }
    }
